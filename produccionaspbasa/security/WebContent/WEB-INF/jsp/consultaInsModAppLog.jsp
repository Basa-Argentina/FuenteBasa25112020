...
<title><spring:message code="formularioInsModAppLog.titulo"
			...
			<fieldset style="border: none; text-align: left; width: 97%;">
				<legend>
					<font class="tituloForm" size="5"> <spring:message
							code="formularioInsModAppLog.titulo" htmlEscape="true" />
					</font>
				</legend>
				<br />
				<form:form action="filtrarInsModAppLog.html"
					commandName="insErrBusqueda" method="post" name="insErrBusqueda">
					<fieldset>
						<table width="100%">
							<thead>
								<tr>
							...
							<table class="busqueda"
								...
								<tr>
									<td align="center">
										<table>
											<tr>
												<td class="texto_ti" style="padding-left: 50px;"><spring:message
														code="formularioInsModAppLog.app" htmlEscape="true" /></td>
												<td class="texto_ti" style="padding-left: 50px;"><spring:message
														code="formularioInsModAppLog.nivel" htmlEscape="true" /></td>
												<td class="texto_ti" style="padding-left: 50px;"><spring:message
														code="formularioInsModAppLog.fecha" htmlEscape="true" /></td>
												<td rowspan="2"
													style="vertical-align: bottom; padding-left: 50px;">
											...
											<tr>
												<td class="texto_ti" style="padding-left: 50px;"><input
													type="text" id="app" name="app" style="width: 200px"
													value="<c:out value="${insModAppLogBusqueda.app}" default="" />" />
												</td>
												<td class="texto_ti" style="padding-left: 50px;"><select
													id="nivel" name="nivel" style="width: 100px;">
														<option value=""></option>
														<option
															<c:if test="${insModAppLogBusqueda.nivel=='ERROR'}"> selected="selected" </c:if>
															value="ERROR">ERROR</option>
														<option
															<c:if test="${insModAppLogBusqueda.nivel=='FATAL'}"> selected="selected" </c:if>
															value="FATAL">FATAL</option>
														<option
															<c:if test="${insModAppLogBusqueda.nivel=='WARN'}"> selected="selected" </c:if>
															value="WARN">WARN</option>
												</select></td>
												<td class="texto_ti" style="padding-left: 50px;"><input
													type="text" id="fechaHora" name="fechaHora" maxlength="10"
				...
				<fieldset>
					<div style="width: 100%; overflow-y: hidden; overflow-x: auto;">
						<display:table name="insModAppLogs" id="insModAppLog"
							requestURI="mostrarInsModAppLog.html" pagesize="7" sort="list">
							<display:column property="app"
								titleKey="formularioInsModAppLog.app" sortable="true"
								class="celdaAlineadoIzquierda" />
							<display:column sortProperty="nivel"
								titleKey="formularioInsModAppLog.nivel" sortable="true"
								class="celdaAlineadoCentrado">
								<a href="#" onclick="consultarError(${insModAppLog.id});"> <c:if
										test="${insModAppLog.nivel == 'ERROR'}">
										<img src="<%=request.getContextPath()%>/images/error.png"
											title="ERROR">
									</c:if> <c:if test="${insModAppLog.nivel == 'FATAL'}">
										<img src="<%=request.getContextPath()%>/images/fatal.png"
											title="FATAL">
									</c:if> <c:if test="${insModAppLog.nivel == 'WARN'}">
										<img src="<%=request.getContextPath()%>/images/warning.png"
											title="WARN">
									</c:if>
								</a>
							</display:column>
							<display:column property="fechaHoraStr"
								titleKey="formularioInsModAppLog.fecha" sortable="true"
								class="celdaAlineadoIzquierda" />
							<display:column property="mensaje"
								titleKey="formularioInsModAppLog.mensaje" sortable="true"
								class="celdaAlineadoIzquierda" />
							<display:column property="clase"
								titleKey="formularioInsModAppLog.clase" sortable="true"
								class="celdaAlineadoIzquierda" />
						</display:table>
					</div>
					<div id="eliminarDiv" style="width: auto; display: block;"
						align="right">
