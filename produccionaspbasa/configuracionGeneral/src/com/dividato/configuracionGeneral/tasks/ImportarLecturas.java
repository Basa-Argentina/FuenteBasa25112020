...
public class ImportarLecturas {
	...
	private static RemitoService remitoService;
	...
	public synchronized void searchFiles() {
		...
		for (File txtFile:lecturas){
			...
			try {
				...
				String[] parts = txtFile.getName().split("-");
			...
			/*	//: Verificar de hacer una validacion de lectura repetido en base
						...
						codigoCorrecto = codigoTomado12 + String.valueOf(EAN13.EAN13_CHECKSUM(codigoTomado12));
								...
								if (codigo.startsWith("99")) {
									...
									if (elemento != null) {
										...
										lecturaDetalle.setEstado_id(0L);
				...
				}else if (tipolectura.equals("0002")){
					Long remito_id  = buscarIdRemito("1"+codRemito,clienteAsp);
					String lectura1  = lecturaService.obtenerPorRemito_id(remito_id);
					if (!lectura1.equals("1")) {

						eliminarRemitoSiexistente(lectura1);
					}
					
					lectura.setTipo(2L);
					
					lectura.setRemito_id(remito_id);
				...
				txtFile.renameTo(new File(getLecturasProcessed()+"//"+nombreLectura+"_"+codigoLectura+"_OK.txt"));	
	...
	private void eliminarRemitoSiexistente(String lectura_id) {
		...
		Boolean eliminar = lecturaService.eliminarporSql(lectura2);
