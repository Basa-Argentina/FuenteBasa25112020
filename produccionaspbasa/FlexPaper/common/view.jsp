...
<%
	...
	String messages 	= "";
	String swfFilePath 	= conf.separate(conf.getConfig("path.swf", "")) + ruta + swfdoc;
	String pdfFilePath 	= conf.separate(conf.getConfig("path.pdf", "")) + ruta + pdfdoc;
	String pdfSplitPath = conf.separate(conf.getConfig("path.swf", "")) + ruta + pdfdoc + "_" + pages + ".pdf";
	String pngFilePath 	= conf.separate(conf.getConfig("path.swf", "")) + ruta + pngdoc;
	String jpgCachePath 	= conf.separate(conf.getConfig("path.swf", "")) + ruta + jpgcachedoc;
	String jsonFilePath 	= conf.separate(conf.getConfig("path.swf", "")) + ruta + jsondoc;
	...
	if(!validatedConfig){
		...
		if("swf".equals(format) || "jpg".equals(format) || "png".equals(format) || "pdf".equals(format)){
			if(!conf.file_exists(swfFilePath)){
				...
				messages = pdfconv.convert(pdfdoc, pages, ruta);
			}
