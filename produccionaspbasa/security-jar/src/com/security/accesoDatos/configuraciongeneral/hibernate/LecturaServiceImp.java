...
public class LecturaServiceImp extends GestorHibernate<Lectura> implements LecturaService {
	...
	public String obtenerPorRemito_id(Long remito_id) {
		...
		try {
			//obtenemos una sesión
			session = getSession();
			String consulta = "SELECT TOP (1) id FROM lecturas where remito_id = "+remito_id+"";
	...
	@Override
	public Boolean eliminarporSql(Long idlectura){
		Session session=null;
		
		try{
			session = getSession();
           	
        	String consulta1 = "DELETE  FROM lecturaDetalle WHERE  (lectura_id ="+idlectura+") ";
        	String consulta = "DELETE  FROM lecturas WHERE  (id ="+idlectura+") ";

        	
        	SQLQuery q1 = session.createSQLQuery(consulta1);
        	SQLQuery q = session.createSQLQuery(consulta);	
