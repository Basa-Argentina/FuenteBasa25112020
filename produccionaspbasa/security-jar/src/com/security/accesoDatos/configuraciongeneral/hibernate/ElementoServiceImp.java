...
public class ElementoServiceImp extends GestorHibernate<Elemento> implements ElementoService {
	...
	public Boolean actualizaTipoTrabajo(Long id, String tipoTrabajo) {
		...
		try {
			...
			Query query = session
					.createSQLQuery("UPDATE elementos ele SET tipoTrabajo='" + tipoTrabajo + "' WHERE id =" + id + "");
	...
	@Override
	public Elemento getContenedorByCodigo(String codigo, String codigoCliente, Boolean limitarCliente,
			ClienteAsp clienteAsp, String tipoCodigoElemento) {
	...
	public Elemento getContenedorByCodigo(String codigo, String codigoCliente, Boolean limitarCliente,
		...
		try {
			...
			String consulta = "SELECT e FROM Elemento e " + " LEFT JOIN e.clienteEmp cli "
					+ " where e.tipoElemento.contenedor = true " + " and e.codigo = '" + codigo + "'";
	...
	public Elemento getProxContenedorDisponByTipoContenedor(String codigo, String codigoCliente, Boolean limitarCliente,
		...
		try {
			...
			String consulta = "SELECT Distinct e FROM Elemento e " + " LEFT JOIN e.clienteEmp cli "
	...
	@Override
	public Elemento getElementoByCodigo(String codigo, String codigoCliente, ClienteAsp clienteAsp,
		...
		try {
			...
			String consulta = "SELECT e FROM Elemento e " + " INNER JOIN e.tipoElemento tip "
					+ " LEFT JOIN e.clienteEmp cli " + " where tip.contenido = true " + " and e.codigo = '" + codigo
					+ "'";
			...
			if (soloLibresOmitirLoteId != null) {
				consulta += " and e.id not in (select ref.elemento.id from Referencia ref where ref.loteReferencia.id <> "
						+ soloLibresOmitirLoteId + " and ref.elemento is not Null ) ";
			}
	...
	@Override
	public Elemento getContenedorOElementoByCodigo(String codigo, String codigoCliente, ClienteAsp clienteAsp,
			Boolean limitarCliente, Long soloLibresOmitirLoteId) {
		Session session = null;
		try {
			// obtenemos una sesion
			...
			if (limitarCliente != null && limitarCliente == true) {
				consulta += "AND elem.clienteEmp.id IN (SELECT cli.id FROM ClienteEmp cli " + "WHERE cli.codigo = '"
						+ codigoCliente + "' ";
			} else {

				consulta += "AND (elem.clienteEmp.id IN (SELECT cli.id FROM ClienteEmp cli " + "WHERE cli.codigo = '"
						+ codigoCliente + "' ) " + "OR elem.clienteEmp IS NULL )";
			}
			if (soloLibresOmitirLoteId != null) {
				consulta += "AND elem.id NOT IN (SELECT ref.id FROM Referencia ref " + "WHERE ref.loteReferencia.id = "
						+ soloLibresOmitirLoteId + "AND ref.elemento IS NOT NULL ";
			}
	...
	//: Generar el motodo obtenerProximoContenedorDisponible
	//

	@Override
	public Elemento obtenerProximoElementoDisponible(ClienteAsp clienteAsp, String codigoCliente,
			List<String> codigosElementoUtilizados) {
		Session session = null;
		try {
			// obtenemos una sesion
			session = getSession();
			...
			if (codigoCliente != null && !codigoCliente.equals("")) {
				...
				crit.add(Restrictions.or(Property.forName("clienteEmp").in(subCritClienteEmp),
						Restrictions.isNull("clienteEmp")));
			}
		...
		try {
			...
			if (elementoBusqueda != null) {
				...
				if ((elementoBusqueda.getCodigoEmpresa() != null && elementoBusqueda.getCodigoEmpresa().length() > 0)
						|| (elementoBusqueda.getCodigoCliente() != null
								&& elementoBusqueda.getCodigoCliente().length() > 0)) {
					crit.createCriteria("clienteEmp", "cli");
					if (elementoBusqueda.getCodigoEmpresa() != null
							&& elementoBusqueda.getCodigoEmpresa().length() > 0) {
						crit.createCriteria("cli.empresa", "emp");
						crit.add(Restrictions.eq("emp.codigo", elementoBusqueda.getCodigoEmpresa()));
					}
					if (elementoBusqueda.getCodigoCliente() != null
							&& elementoBusqueda.getCodigoCliente().length() > 0) {
						crit.add(Restrictions.eq("cli.codigo", elementoBusqueda.getCodigoCliente()));
					}

				}
				if (elementoBusqueda.getCodigoTipoElemento() != null
						&& elementoBusqueda.getCodigoTipoElemento().length() > 0) {
	...
	public List<Elemento> listarElementoFiltradas(Elemento elemento, ClienteAsp clienteAsp, Boolean impresion) {
		...
		try {
			...
			// obtenemos una sesion
			session = getSession();
			// Si es null retornamos en cero, se puso dentro de session debido a que debe
			// cerrar el finally
			...
			if (elemento != null) {
				...
				if (elemento.getCodigoCliente() != null && !"".equals(elemento.getCodigoCliente())) {
					...
					if (elemento.getCajasVacias() != null && elemento.getCajasVacias())
						crit.add(Restrictions.or(Restrictions.isNull("clienteEmp"),
								Restrictions.eq("clienteEmp", elemento.getClienteEmp())));
					else {
				...
				if (elemento.getEstado() != null && !"".equals(elemento.getEstado())
						&& !"Seleccione un Estado".equals(elemento.getEstado())) {
				...
				if (elemento.getCodigoLectura() != null && !"".equals(elemento.getCodigoLectura())) {
					...
					Lectura lectura = lecturaService.obtenerPorCodigo(Long.parseLong(elemento.getCodigoLectura()),
							clienteAsp);
					if (lectura != null) {
						List<LecturaDetalle> listaDetalles = lecturaDetalleService
								.listarLecturaDetallePorLectura(lectura, clienteAsp);
		...
		try {
			...
			if (elemento != null) {
				...
				if (elemento.getCodigoLectura() != null && !"".equals(elemento.getCodigoLectura())) {
					...
					if (lectura != null) {
						List<LecturaDetalle> listaDetalles = lecturaDetalleService
								.listarLecturaDetallePorLectura(lectura, cliente);
						if (listaDetalles != null && listaDetalles.size() > 0) {
							Disjunction disjunction = Restrictions.disjunction();
							for (LecturaDetalle detalle : listaDetalles) {
								if (detalle.getElemento() != null) {
...
//	        	//Ordenamos
	...
	public List<Elemento> listarElementoPopupPaginado(ClienteAsp cliente, Elemento elemento,
			...
			String codigoCliente, Boolean limitarDependencia, Long libresODistintoLoteId, Boolean esContenedor) {

		return listarElementoPopupPaginado(cliente, elemento, descripcionTipoElemento, codigoTipoElemento,
				prefijoCodigoTipoElemento, codigoCliente, limitarDependencia, libresODistintoLoteId, esContenedor,
				null);
	...
	public List<Elemento> listarElementoPopupPaginado(ClienteAsp cliente, Elemento elemento,
		...
		Session session = null;
		try {
			List<Long> ids = obtenerIDsElementoPopupPaginado(cliente, elemento, descripcionTipoElemento,
					codigoTipoElemento, prefijoCodigoTipoElemento, codigoCliente, limitarDependencia,
					libresODistintoLoteId, esContenedor, cerrado);
			// obtenemos una sesion
			session = getSession();
			// Si es null retornamos en cero, se puso dentro de session debido a que debe
			// cerrar el finally
	...
	private List<Long> obtenerIDsElementoPopupPaginado(ClienteAsp cliente, Elemento elemento,
		...
		try {
			...
			if (elemento.getCodigoTipoElemento() != null && !elemento.getCodigoTipoElemento().equals("")
					&& !"undefined".equalsIgnoreCase(codigoTipoElemento)) {
			...
			if (clienteEmp != null) {
				if (limitarDependencia != null && limitarDependencia == true)
					...
					crit.add(Restrictions.or(Restrictions.eq("clienteEmp", clienteEmp),
							Restrictions.isNull("clienteEmp")));
			} else {
	...
	@Override
	public Integer contarElementoPopupPaginado(ClienteAsp cliente, String descripcionTipoElemento,
			String codigoTipoElemento, String prefijoCodigoTipoElemento, String codigoCliente,
			Boolean limitarDependencia, Long libresODistintoLoteId, Boolean esContenedor) {

		return contarElementoPopupPaginado(cliente, descripcionTipoElemento, codigoTipoElemento,
				prefijoCodigoTipoElemento, codigoCliente, limitarDependencia, libresODistintoLoteId, esContenedor,
				null);
	}
	...
	public Integer contarElementoPopupPaginado(ClienteAsp cliente, String descripcionTipoElemento,
			String codigoTipoElemento, String prefijoCodigoTipoElemento, String codigoCliente,
			Boolean limitarDependencia, Long libresODistintoLoteId, Boolean esContenedor, Boolean cerrado) {
		...
		try {
			...
			// filtro tipo de elemento
			if (codigoTipoElemento != null && !codigoTipoElemento.equals("")
					&& !"undefined".equalsIgnoreCase(codigoTipoElemento)) {
	...
	@Override
	public synchronized Boolean guardarReposicionamiento(List<Posicion> posicionesOrigen,
			List<Posicion> posicionesDestino, List<Elemento> elementosReposicionados,
			List<Elemento> elementosAnterioresModuloDestino, ClienteAsp clienteAsp, User user, Lectura lectura) {
		Session session = null;
		Transaction tx = null;
		...
		try {
			...
			// recorremos la lista de elementos ubicados en el modulo destino antes del
			// reposicionamiento
			...
			for (Elemento e : elementosAnterioresModuloDestino) {
				movimiento = crearMovimientoTransferenciaParaActualizacionElementosAnterioresModuloDestino(e,
						clienteAsp, user, date, lectura);
				movimiento.setEstadoElemento(Constantes.ELEMENTO_ESTADO_EN_EL_CLIENTE);
			...
			List<Elemento> elementosActualesPosicionesOrigen = obtenerElementosAsignadosAPosicionesOrigen(
					elementosReposicionados, posicionesOrigen);
			List<Posicion> posicionesOrigenALiberar = obtenerListaPosicionesOrigenNoOcupadaPorOtroElemento(
					posicionesOrigen, elementosActualesPosicionesOrigen);
			for (Posicion posOrigen : posicionesOrigenALiberar) {
				if (verificarPosicionAnteriorElementoReposicionadoNoPerteneceModuloDestino(posOrigen,
						posicionesDestino)) {
					session.update(posOrigen);
				}
			}

			// recorremos la lista de elementos a reposicionar actualizamos su estado y su
			// posicion,
	...
	@Override
	public synchronized Boolean guardarAsignacionPosicionesLibres(List<Posicion> posicionesOrigen,
			List<Posicion> posicionesDestino, List<Elemento> elementosReposicionados, ClienteAsp clienteAsp, User user,
			Lectura lectura) {
		Session session = null;
		Transaction tx = null;
		Boolean result = false;
		...
		try {
			...
			Boolean siguenDisponibles = posicionService.verificarEstadoPosiciones(posicionesDestino, clienteAsp,
					Constantes.POSICION_ESTADO_DISPONIBLE);
			...
			// y guardamos los movimientos correspondientes
			for (Elemento elemento : elementosReposicionados) {
				elemento.setDepositoActual(
						elemento.getPosicionFutura().getEstante().getGrupo().getSeccion().getDeposito());
				movimiento = crearMovimientoReposicionamiento(elemento, clienteAsp, user, date, lectura);
	...
	@SuppressWarnings("unchecked")
	public synchronized List<Posicion> obtenerPosicionesAnterioresPorElementos(List<Elemento> elementosReposicionados,
		...
		try {
			...
			if (clienteAsp != null) {
				crit.createCriteria("posicion").createCriteria("modulo").createCriteria("estante")
						...
						.createCriteria("sucursal").createCriteria("empresa")
						.add(Restrictions.eq("cliente", clienteAsp));
			}
			crit.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);
			elemRep = crit.list();
	...
	public synchronized Boolean verificarTipoElementoValidoParaGrupoDeModulos(TipoElemento tipoElemento,
			Modulo moduloDestino, ClienteAsp clienteAsp) {
		Session session = null;
		int cont = 1;
		// Buscamos en la base de datos la cantidad de elementos con tipo diferente al
		// ingresado por parametro
		// que esten colocados en posiciones correspondientes a modulos adyacentes al
		// modulo destino.
		// Si la consulta devuelve 0 entonces los modulos adyacentes estan vacios o
		...
		try {
			if (clienteAsp == null) {
				throw new ClienteAspNullException(
						"No se puede ejecutar el metodo verificarTipoElementoValidoParaGrupoDeModulos porque clienteAsp = null");
			}
			// calculamos coordenadas de posiciones del modulo
			int horPorModulo = moduloDestino.getEstante().getGrupo().getHorizontales()
					/ moduloDestino.getEstante().getGrupo().getModulosHor();
			int vertPorModulo = moduloDestino.getEstante().getGrupo().getVerticales()
					/ moduloDestino.getEstante().getGrupo().getModulosVert();
			...
			c.createCriteria("pos.estante", "est")
					.add(Restrictions.eq("est.grupo", moduloDestino.getEstante().getGrupo())).createCriteria("grupo")
					.createCriteria("seccion").createCriteria("deposito").createCriteria("sucursal")
					.createCriteria("empresa", "emp").add(Restrictions.eq("emp.cliente", clienteAsp));
	...
	@Override
	public synchronized Boolean verificarTipoElementoValidoParaMismoModulo(TipoElemento tipoElemento,
			Modulo moduloDestino, ClienteAsp clienteAsp) {
		Session session = null;
		int cont = 1;
		// Buscamos en la base de datos la cantidad de elementos con tipo diferente al
	...
	@SuppressWarnings("unchecked")
	public synchronized List<Elemento> buscarElementosAnterioresModuloDestino(Modulo moduloDestino,
			ClienteAsp clienteAsp) {
		Session session = null;
		List<Elemento> result = null;
		try {
			...
			c.createCriteria("posicion.estante").createCriteria("grupo").createCriteria("seccion")
					.createCriteria("deposito").createCriteria("sucursal").createCriteria("empresa", "emp")
	...
	@SuppressWarnings("unchecked")
	public List<Elemento> listarElementoPopup(String val, String codigoDeposito, String codigoTipoElemento,
			String codigoClienteEmp, ClienteAsp clienteAsp) {
		Session session = null;
		List<Elemento> result = null;
	...
	@Override
	public Long cantidadElementosPorConceptoFacturable(Long idClienteEmp, Long idConceptoFacturable,
			Boolean generaCostoXGuarda, ClienteAsp clienteAsp) {
		Session session = null;
		Long result = null;
	...
	/**
	 * Metodo que devuelve el listado de elementos filtrado, pero realiza la
	 * busqueda por consulta SQL.
	 * 
	 * @author Victor Kenis
	 * @return
	 */
	...
	public List<Elemento> traerElementosRetiroPorSQL(Elemento elemento, ClienteAsp clienteAsp, String clasificaciones) {
		...
		try {
			...
			String consulta = "select ele.* " + "from elementos ele "
			...
			consulta += "where 1=1 " + "and ele.clienteAsp_id =" + clienteAsp.getId() + " " + "AND ele.clienteEmp_id ="
					+ elemento.getClienteEmp().getId().longValue() + " " + "and Exists(SELECT 1 " + "from referencia "
			...
			if (elemento.getCodigoLectura() != null && !elemento.getCodigoLectura().equals(""))
				consulta += " AND ele.id IN (SELECT lecturaDetalle.elemento_id FROM lecturaDetalle "
						+ " Left JOIN lecturas lec on lecturaDetalle.lectura_id = lec.id WHERE lec.codigo = '"
						+ elemento.getCodigoLectura() + "') ";
	...
	public Boolean revisarReferencias(Long id) {
		...
		try {
			...
			String consulta = "SELECT elementos.id, elementos.basaReferencia, elementos.basaIdCaja, referencia.elemento_id "
					+ " FROM  elementos LEFT OUTER JOIN " + " referencia ON elementos.id = referencia.elemento_id "
	...
	public Integer contarElementosRetiroPorSQL(Elemento elemento, ClienteAsp clienteAsp, String clasificaciones) {
		...
		try {
			...
			String consulta = "select count(distinct ele.id) " + "from elementos ele "
					+ " LEFT JOIN tipoElementos AS tip ON ele.tipoElemento_id = tip.id ";
	...
	public List<Long> traerIdElementosEnLectura(String codigoLectura, ClienteAsp clienteAsp) {
		...
		try {
			...
			String consulta = "SELECT ele.id " + " From elementos ele " + " where ele.clienteAsp_id ="
					+ clienteAsp.getId() + " "
					+ " AND ele.id IN (SELECT lecturaDetalle.elemento_id FROM lecturaDetalle "
					+ " Left JOIN lecturas lec on lecturaDetalle.lectura_id = lec.id WHERE lec.codigo = '"
					+ codigoLectura + "') " + " ORDER BY ele.id ";
	...
	public List<Elemento> traerElementosPorSQL(Elemento elemento, ClienteAsp clienteAsp) {
		...
		try {
			...
			String consulta = "SELECT ele.* " + " From elementos ele "
					+ " LEFT JOIN tipoElementos AS tip ON ele.tipoElemento_id = tip.id "
					+ " LEFT JOIN clientesEmp cli ON ele.clienteEmp_id = cli.id and ele.clienteAsp_id ="
					+ clienteAsp.getId() + " " + " LEFT JOIN empresas emp ON cli.empresa_id = emp.id "
					+ " LEFT JOIN sucursales AS suc ON emp.id = suc.empresa_id "
					+ " LEFT JOIN depositos dep ON ele.depositoActual_id = dep.id "
					+ " LEFT JOIN elementos ee ON ele.contenedor_id = ee.id " + " WHERE 1=1 ";
	...
	//////////////////////////////// REPOSICIONAMIENTO///////////////////////////
	/**
	 * Crea un objeto movimietnto de tipo transferencia a partir de los valores de
	 * los campos posicion y posicion futura del elemento pasado por parametro
	 * 
	 * @param elementoReposicionado
	 * @return
	 */
	private Movimiento crearMovimientoTransferencia(Elemento elementoReposicionado, ClienteAsp clienteAsp, User user,
			Date date, Lectura lectura) throws PosicionDestinoNullEnReposicionamientoException {
	 ...
	 * movimiento a partir de elementos que tienen seteado el deposito pero no
	 * tienen seteada la posicion
	 * 
	 * @param elementoReposicionado
	 * @return
	 */
	private Movimiento crearMovimientoTransferenciaPorIngreso(Elemento elementoReposicionado, ClienteAsp clienteAsp,
			User user, Date date, Lectura lectura) throws PosicionDestinoNullEnReposicionamientoException {
		if (elementoReposicionado.getPosicionFutura() == null) {
			throw new PosicionDestinoNullEnReposicionamientoException();
		}
	 ...
	 */
	private Movimiento crearMovimientoReposicionamiento(Elemento elemento, ClienteAsp clienteAsp, User user, Date date,
			Lectura lectura) throws ElementoDepositoLocalDistintoDepositoDestinoReposicionamientoException,
			PosicionDestinoNullEnReposicionamientoException {
	...
	private Movimiento crearMovimientoTransferenciaParaActualizacionElementosAnterioresModuloDestino(
			Elemento elementoAnteriorModuloDestino, ClienteAsp clienteAsp, User user, Date date, Lectura lectura)
			 {
	...
	/**
	 * Verifica que existan posiciones Origen para los elementos a reposicionar, y
	 * que dichas posiciones no esten ocupadas por nuevos elementos.
	 * 
	 * @param elementos
	...
	/**
	 * Devuelve una lista con los elementos (diferentes a los elementos a
	 * reposicionar) que se encuentran en alguna (o todas) las posiciones de origen
	 * de los elementos a reposicionar.
	 * 
	 * @param elementosReposicionados Los elementos a reposicionar
	 * @param posicionesOrigen        Las posiciones de origen de los elementos a
	 *                                reposicionar
	 * @return
	 */
	@SuppressWarnings("unchecked")
	private List<Elemento> obtenerElementosAsignadosAPosicionesOrigen(List<Elemento> elementosReposicionados,
			List<Posicion> posicionesOrigen) {
		List<Elemento> result = null;
		Session session = null;
		if (posicionesOrigen != null && posicionesOrigen.size() > 0) {
			try {
	...
	/**
	 * Devuelve una lista con las posiciones que deben liberarce durante el proceso
	 * de reposicionamiento
	 * 
	 * @param posicionesOrigen                    Las posiciones de origen de los
	 *                                            elementos a reposicionar
	 * @param elementosAsignadosAPosicionesOrigen Los elementos no pertenecientes al
	 *                                            conjunto de elementos a
	 *                                            reposicionar, que tienen asignadas
	 *                                            posiciones incluidas en la lista
	 *                                            de posiciones de origen
	 * @return
	 */
	private List<Posicion> obtenerListaPosicionesOrigenNoOcupadaPorOtroElemento(List<Posicion> posicionesOrigen,
			List<Elemento> elementosAsignadosAPosicionesOrigen) {
	...
	/**
	 * Verifica que un elemento ubicado anteriormente en el modulo destino sea
	 * diferente a los elementos a reposicionar, de esta forma se evita guardar el
	 * campo posicion del elemento a null y luego a la posicion destino en la misma
	 * sesion de hibernate, lo que provoca una exception
	 * 
	 * @param elementoAnteriorModuloDestino
	 * @param elementosReposicionados
	 * @return
	 */
	private boolean verificarElementoAnteriorModuloOrigenDistintoElementosReposicionados(
			Elemento elementoAnteriorModuloDestino, List<Elemento> elementosReposicionados) {
		boolean result = true;
		for (Elemento e : elementosReposicionados) {
	...
	/**
	 * Verifica que una posicion anterior de alguno de los elementos reposicionados
	 * no pertenezca al modulo destino, de esta forma se evita liberar y luego
	 * ocupar una posicion en la misma sesion de hibernate lo que provoca una
	 * exception
	 *
	 * @return
	 */
	private boolean verificarPosicionAnteriorElementoReposicionadoNoPerteneceModuloDestino(Posicion posicionOrigen,
	...
	private ElementoHistorico registrarHistoricoElementos(String mensaje, Elemento elemento, User user,
			ClienteAsp clienteAsp) {
