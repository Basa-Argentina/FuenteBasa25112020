...
public class LecturaDetalleServiceImp extends GestorHibernate<LecturaDetalle> implements LecturaDetalleService {
	...
	@Override
	public List<LecturaDetalle> listarDetalleLecturaPorSql(Long idlectura){
		Session session=null;
		
		try{
			session = getSession();
           	
        	String consulta = "SELECT   id, codigoBarras, observacion, orden, elemento_id, lectura_id, estado_id, remito\r\n" + 
        			"FROM            lecturaDetalle\r\n" + 
        			"WHERE        (lectura_id = :lecturaid) ";
        			        	
        	SQLQuery q = session.createSQLQuery(consulta);			
        	q.setLong("lecturaid", idlectura);
			return q.list();
